...
for char in letters:
    #this loops through every character in the letters list 
    #this is needed because each letter needs its own folder
    char_dir = os.path.join("train", char)
    #this creates the folder path 
    os.makedirs(char_dir, exist_ok=True)
    #this is so that way this works on all operating systems 
    for i in range(50):
        #50 is different versions of the character 
        #before, I had 10 which meant that there was less variation and less learning 
        # Create a blank 20x20 grayscale image
        img = Image.new('L', (20, 20), color=255)
        draw = ImageDraw.Draw(img)
        x_position = np.random.randint(5, 15)
        #this creates a random horizontal position 
        y_position = np.random.randint(2, 10)
        #this creates a random vertical position 
        ...
        img.save(os.path.join(char_dir, f"{char}_{i}.jpg"))

print("Created synthetic 'train' folder with character images!")
    ...
    if region_height >= min_height and region_height <= max_height and region_width >= min_width and region_width <=max_width: 
        ...
        if 1.5 <= aspect_ratio <= 5.0:
            if region.area > max_area:
                ...
                best_plate = binary_car_image[min_row:max_row, min_col:max_col]
...
print("Plate size:", license_plate.shape)
fig2, ax_plate = plt.subplots(1)
license_plate = clear_border(license_plate)
license_plate = binary_closing(license_plate, square(2))
#this creates a new display window and ax_plate is the drawing canvas 
ax_plate.imshow(license_plate, cmap = "gray")
character_dimensions = (0.20*license_plate.shape[0], #min height 
                        0.60 *license_plate.shape[0], #max height 
                        0.02*license_plate.shape[1], #min width 
                        0.35 * license_plate.shape[1] #max width
                        )
...
for region in regionprops(labelled_plate):
    if region.area < 5 or region.area > 100:
        continue
    y0, x0, y1, x1 = region.bbox
    region_height = y1-y0
    region_width = x1 - x0 
    print("Candidate:", region_height, region_width)
...
print("Character positions:", column_list)
# combine each character with its x position
character_pairs = list(zip(column_list, characters))

# sort characters from left to right
character_pairs.sort(key=lambda x: x[0])

# separate them back out
column_list = []
characters = []

for position, char in character_pairs:
